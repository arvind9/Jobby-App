...
Test Case 51: Page should consist of at least two HTML list items, jobs list received from the response and the provided employmentTypesList, salaryRangesList should be rendered using a unique key as a prop for each employment type item, salary range item and job item respectively
...
Test Case 76: When a value is provided in the HTML input element for search and search icon button is clicked, an HTTP GET request should be made with the value provided in the search input element as the value to query parameter "search"
↳ TestingLibraryElementError: Unable to find an accessible element with the role "searchbox"


Test Case 77: When employment types are selected, an HTTP GET request should be made with the ids of the employment types as a single string separated by "," as value to query parameter "employment_type"
↳ TestingLibraryElementError: Unable to find a label with the text of: /Full Time/i


Test Case 78: When a salary range is clicked, an HTTP GET request should be made with the id of the salaryRange as value to query parameter "minimum_package"
↳ TestingLibraryElementError: Unable to find a label with the text of: /10 LPA/i


Test Case 79: When all filters are used, an HTTP GET request should be made to jobsApiUrl with all the query parameters and their values
↳ TestingLibraryElementError: Unable to find an accessible element with the role "searchbox"


Test Case 115: When a random path is provided as the URL path, then the page should be navigated to the Not Found Route and should consist of an HTML image element with alt attribute value as "not found" and src as the given not found image URL
↳ 
Expected: "/not-found"
Received: "/bad-path"